// src/pages/Dept/DeptDetails.jsx
...
const DeptDetails = () => {
  ...
  const returnTo = location.state?.returnTo || "/dept/slot-booking";
  ...
  // Normalizer: ensure hall has expected fields (location extraction removed from UI usage)
  const normalizeHall = (h) => ({
    id: h.id ?? h._id ?? h.hallId ?? `hall-${Math.random().toString(36).slice(2)}`,
    name: h.name ?? h.hallName ?? h.title ?? "Unnamed Hall",
    capacity: h.capacity ?? h.size ?? h.capacity_people ?? null,
    photos: Array.isArray(h.photos) ? h.photos : Array.isArray(h.images) ? h.images : [],
    videos: Array.isArray(h.videos)
      ? h.videos
      : Array.isArray(h.media)
      ? (h.media.filter((m) => typeof m === "string" && m.match(/\.(mp4|webm|ogg)$/i)) || [])
      : [],
    features: Array.isArray(h.features) ? h.features : Array.isArray(h.amenities) ? h.amenities : [],
    meta: h.meta ?? {},
    raw: h,
  });
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      const hallsRaw = Array.isArray(resH?.data) ? resH.data : [];
      const seminarsList = Array.isArray(resS?.data) ? resS.data : [];

      const hallsList = hallsRaw.map(normalizeHall);
  ...
  // BOOKED HOURS: only APPROVED seminars should block hours
  useEffect(() => {
    if (!selectedHall) {
    ...
    const daySeminars = seminars.filter(
      (s) =>
        normalizeDateOnly(s.date) === dayStr &&
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      if (viewer.type === "video" && videoRef.current) {
        try {
          ...
          if (requestFS) {
            try { requestFS.call(el); } catch (e) {}
          }
        } catch (err) {
          console.warn("Viewer auto-play/fullscreen failed:", err);
        }
  ...
  }, [viewer.open, viewer.type, viewer.index]);

  useEffect(() => {
    if (!viewer.open || !viewerRef.current) return;
    const el = viewerRef.current;
    let startX = 0, startY = 0, tracking = false;
    const THRESH = 40;
    const onTouchStart = (e) => { if (!e.touches || !e.touches.length) return; startX = e.touches[0].clientX; startY = e.touches[0].clientY; tracking = true; };
    const onTouchMove = (e) => { if (!tracking || !e.touches || !e.touches.length) return; const dx = e.touches[0].clientX - startX; const dy = e.touches[0].clientY - startY; if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > 6) e.preventDefault(); };
    const onTouchEnd = (e) => { if (!tracking) return; tracking = false; const touch = (e.changedTouches && e.changedTouches[0]) || null; if (!touch) return; const dx = touch.clientX - startX; const dy = touch.clientY - startY; if (Math.abs(dx) > Math.abs(dy) && Math.abs(dx) > THRESH) { if (dx < 0) goNext(); else goPrev(); } };
  ...
  const hourClicked = (idx) => {
    if (bookedHours.has(idx)) {
      ...
      const owners = seminars.filter((s) => {
        ...
        if (s.hallName !== selectedHall) return false;
        // only APPROVED owners should be considered when showing booking info
      ...
      if (owners.length > 0) {
        ...
        showNotification("🛑 Slot already booked", 4000);
      }
      return;
    }
    if (!selectedRange) {
          ...
          <main className="lg:col-span-3 bg-white rounded-lg shadow p-4 sm:p-6">
            ...
            <section className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                {/* centered constrained card so pills don't stretch full width */}
                <div className="bg-white p-4 rounded-lg shadow-sm">
                  <div
                    className="mx-auto"
                    style={{
                      // limit width of the pill card on large screens, center it
                      maxWidth: 980,
                    }}
                  >
                    {/* auto-fit grid: each pill at least 80px wide, grows to fill available space (no overlapping/stretching) */}
                    <div
                    ...
                    >
                      {HOURS.map((h) => {
                        ...
                        // pill base: balanced padding, rounded corners
                        ...
                        const bookedCls = `${base} ${pillPadding} rounded-2xl bg-rose-50 text-rose-700 border border-rose-100 shadow-sm`;
                        ...
                        return (
                          <button
                            key={h.index}
                            type="button"
                            onClick={() => hourClicked(h.index)}
                            title={isBooked ? "Already booked" : `${h.label}`}
                            className={cls}
